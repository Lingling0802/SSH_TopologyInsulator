...
PROGRAM Hamiltion
  ...
  INTEGER m1,m2,q1,q2,N1,N2,nx
  INTEGER xsA,ysA,xsB,ysB,xoA,yoA,xoB,yoB,xA,yA,xB,yB
  REAL*8 Dde(N,N),Dda(N,N),Dy,dx(2*N),Nr
  DOUBLE COMPLEX de(N),da(N),ab(N),ac(N),b(N),c(N)
  DOUBLE COMPLEX Vm(2*N),Valp(N),Vbat(N),PsiA(N,N),PsiB(N,N),Phi(2*N,2*N)
  REAL*8 d,tp,tm,kx,k1,Ve,kxx(Mx)
  REAL*8 HI(2*N,2*N),HR(2*N,2*N),wr(2*N),wi(2*N),zr(2*N,2*N),zi(2*N,2*N),vr(N,N),vi(N,N)
  REAL*8 fv1(2*N),fv2(2*N),fv3(2*N)
  REAL*8 PhiR(2*N,2*N),PhiI(2*N,2*N),PsiAR(N,N),PsiAI(N,N)
  DOUBLE COMPLEX H(2*N,2*N),ini
  REAL*8 locdos(2*N,2*N),Ew,Ewm,WF(2*N,2*N),tlocdos(2*N,2*N)
...
do nx=1,Mx
    kxx(nx)=((nx*1.d0-16.d0)/(16.d0))*PI
    kx=kxx(nx)
!    kx=PI/4.d0
    k1=kx

  do iy=1,N
     ab(iy)=-tp*(1+de(iy))-tm*CMPLX(COS(kx),-SIN(kx))
     ac(iy)=-tp*(1+de(iy))-tm*CMPLX(COS(kx),SIN(kx))
     b(iy)=-tp*(1-da(iy))*CMPLX(COS(kx),SIN(kx))-tm
     c(iy)=-tp*(1-da(iy))*CMPLX(COS(kx),-SIN(kx))-tm
  end do
...
!       write(26,*) Vm(it)
    ...
    do m1=1,N1
      do m2=1,N2
          PsiA(m1,m2)=(1.d0/SQRT(Nr))*Valp(m2)*CMPLX(COS(k1*m1),SIN(k1*m1))
          PsiB(m1,m2)=(1.d0/SQRT(Nr))*Vbat(m2)*CMPLX(COS(k1*m1),SIN(k1*m1))
     end do
    end do

!  do i=1,N
!     do j=1,N
!        PsiAR(i,j)=REAL(PsiA(i,j))
!        PsiAI(i,j)=AIMAG(PsiA(i,j))
!     end do
!  end do

!  do j=N,1,-1
!      do i=1,N
!         write(100,*) PsiAR(i,j)**2+PsiAi(i,j)**2
!      end do
!   end do
!============================================================================================
!              ------ I ------
!============================================================================================
   do q1=1,N1/2
      do q2=1,q1
        ...
        if(xA.eq.0) then
           xsA=xA+2*N1
         else if(xA.eq.(2*N1+1)) then
           xsA=xA-2*N1
         else if(yA.eq.(2*N2+1)) then
           ysA=yA-2*N2
         end if
         ...
         if(xB.eq.0) then
             xsB=xB+2*N1
         else if(xB.eq.(2*N1+1)) then
             xsB=xB-2*N1
         else if(yB.eq.(2*N2+1)) then
             ysB=yB-2*N2
         end if
...
!        write(10,*) q1,q2,xA,yA,xB,yB,xsA,ysA,xsB,ysB
