...
PROGRAM Hamiltion
  ...
  INTEGER m1,m2,q1,q2,N1,N2,nx,ny
  ...
  REAL*8 t0,alp,locdos(2*N,2*N),Ew,Ewm,WF(2*N,2*N)
...
do nx=1,Mx
  ...
  do iy=1,N
     ...
     b(iy)=-t2(1,iy)*CMPLX(COS(kx),SIN(kx))-t3(1,iy)
     c(iy)=-t2(1,iy)*CMPLX(COS(kx),-SIN(kx))-t3(1,iy)
  end do
...
!   do ix=1,N
!      do iy=1,N
!         vr(ix,iy)=zr(2*iy-1,im)+(-1)**(ix)*zr(2*iy,im)
!         vi(ix,iy)=zi(2*iy-1,im)+(-1)**(ix)*zi(2*iy,im)
!      end do
!   end do
